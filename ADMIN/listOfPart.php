...
class PDF extends TCPDF
{
    public function Header()
    {
		...
		$this->Ln(40);
		$this->Write(0, 'LIST OF PARTICIPANTS', '', 0, 'C', true, 0, false, false, 0);
...
$pdf->Cell(65,5,' Student Name',0,0,'L',1);
$pdf->Cell(30,5,' Event Name',0,0,'L',1);
$pdf->Cell(20,5,' Register',0,0,'L',1);
$pdf->Cell(20,5,' Start',0,0,'L',1);
$pdf->Cell(20,5,' End',0,0,'L',1);

$query1 = "SELECT studentinfotable.studentID,studentinfotable.studentFname,studentinfotable.studentLname,eventregistrationtable.eventName,eventregistrationtable.eventDateStart,eventregistrationtable.eventDateEnd,eventmembershiptable.dateRegister FROM eventmembershiptable INNER JOIN studentinfotable ON eventmembershiptable.memID = studentinfotable.studentID INNER JOIN eventregistrationtable ON eventmembershiptable.eventID = eventregistrationtable.eventID WHERE eventregistrationtable.eventStatus = 'ACTIVE' ; ";
...
while($row = mysqli_fetch_array($result1)) {
	...
	$sStart = $row['eventDateStart'];
	$sEnd = $row['eventDateEnd'];
	$sReg = $row['dateRegister'];
	$eName = $row['eventName'];
    ...
    $pdf->Cell(65,5,' '. $sName,0,0,'L',1);
    $pdf->Cell(30,5,' '. $eName,0,0,'L',1);
    $pdf->Cell(20,5,' '. $sReg,0,0,'L',1);
    $pdf->Cell(20,5,' '. $sStart,0,0,'L',1);
    $pdf->Cell(20,5,' '. $sEnd,0,0,'L',1);
    $i++;
}
